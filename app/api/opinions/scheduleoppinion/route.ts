...
export async function POST(request: NextRequest){
  ...
  if(!jwt_user.user_name) return NextResponse.json({ error: you_need_account_to_post }, { status: 500 });
  if(jwt_user.user_name!=process.env.ADMIN_USERNAME) return NextResponse.json({ error: you_need_account_to_post }, { status: 500 });
